...
// Only importing state for layout and modals
const { 
  activeTab, aiAnalysis, manageAdvice, showBuyModal, 
  selectedToken, buyAmount, isBuying, openBuyModal, executeBuy, 
  closePosition, activeTrades, formatPrice 
} = useTrader()
</script>

<template>
  <div class="bg-slate-800 rounded-2xl shadow-2xl border border-slate-700 overflow-hidden min-h-[80vh] flex flex-col relative">
    
    <!-- (MODALS CODE... Keep exactly as before, using formatPrice in Buy Modal) -->
    <!-- 1. BUY MODAL -->
      ...
      <div class="bg-slate-900 border border-slate-600 w-full max-w-md rounded-2xl p-6 shadow-2xl animate-fade-in">
        <div class="flex justify-between items-start mb-6">
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <p class="text-xs text-slate-400 font-mono">${{ formatPrice(selectedToken.price) }}</p>
            </div>
          </div>
        ...
        <div class="mb-6">
          ...
          <div class="relative">
            ...
            <input type="number" v-model="buyAmount" class="w-full bg-slate-800 border border-slate-600 rounded-xl py-4 pl-8 pr-4 text-2xl font-bold text-white focus:ring-2 focus:ring-green-500 outline-none"/>
          </div>
          <div class="flex gap-2 mt-3">
            <button @click="buyAmount = 10" class="flex-1 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded text-xs font-bold transition-colors">$10</button>
            <button @click="buyAmount = 50" class="flex-1 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded text-xs font-bold transition-colors">$50</button>
            <button @click="buyAmount = 100" class="flex-1 py-2 bg-slate-800 hover:bg-slate-700 border border-slate-600 rounded text-xs font-bold transition-colors">$100</button>
          </div>
        </div>
        <button @click="executeBuy" :disabled="isBuying" class="w-full py-4 rounded-xl font-bold text-lg bg-green-500 hover:bg-green-400 text-black transition-all shadow-lg disabled:opacity-50">
          {{ isBuying ? 'Executing...' : `Confirm Buy ($${buyAmount})` }}
        </button>
      </div>
    </div>

    <!-- 2. MANAGER ADVICE MODAL (Keep as is) -->
    <div v-if="manageAdvice" class="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    ...
    <!-- 3. AI ANALYSIS POPUP (Keep as is) -->
    <div v-if="aiAnalysis && aiAnalysis.token" class="fixed inset-0 bg-black/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    ...
    <!-- HEADER -->
    <div class="p-4 md:p-6 border-b border-slate-700 flex flex-col md:flex-row justify-between items-center gap-4 bg-slate-900/50">
      <div>
        <h1 class="text-2xl md:text-3xl font-bold bg-gradient-to-r from-purple-400 to-pink-500 bg-clip-text text-transparent">
          AITrader <span class="text-xs bg-slate-700 text-white px-2 py-0.5 rounded uppercase ml-2 align-middle">SIMULATION</span>
        </h1>
      </div>
      <ClientOnly><WalletMultiButton dark /></ClientOnly>
    </div>

    <!-- TABS NAVIGATION -->
    <div class="flex border-b border-slate-700 bg-slate-800 overflow-x-auto no-scrollbar">
    ...
    <!-- DYNAMIC CONTENT -->
    <div class="p-4 md:p-6 flex-1 bg-slate-800 overflow-y-auto">
