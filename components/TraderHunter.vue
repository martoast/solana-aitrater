...
const { 
  ...
  analyzeToken, openBuyModal, formatVal, getExplorerLink, formatTimeAgo,isBatchAnalyzing 
} = useTrader()
</script>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div v-else class="grid gap-3">
          <div v-for="token in verifiedTokens" :key="token.address" 
               ...
               :class="{
                 'border-yellow-400 shadow-yellow-900/20 scale-[1.01]': (token.aiScore >= 80), 
                 'border-slate-700': !token.aiScore || (token.aiScore >= 30 && token.aiScore < 80),
                 'border-red-900 opacity-60': token.aiScore && token.aiScore < 30
               }"
          >
            <!-- AI BADGE (FIXED LOGIC) -->
            <div v-if="token.aiScore" class="absolute top-0 right-0 px-2 py-1 text-[10px] font-bold rounded-bl-lg"
                 :class="token.aiScore >= 80 ? 'bg-green-500 text-black' : (token.aiScore < 30 ? 'bg-red-900 text-white' : 'bg-yellow-600 text-black')">
                 <!-- Use aiTag (Batch) OR aiDecision (Auto) -->
                 AI: {{ token.aiScore }}/100 ({{ token.aiTag || token.aiDecision }})
            </div>
            ...
            <div class="flex gap-2">
              <button @click="analyzeToken(token)" :disabled="processingId === token.address" class="flex-1 py-3 bg-slate-700 hover:bg-slate-600 rounded-lg font-bold text-sm disabled:opacity-50 shadow-lg">
                {{ processingId === token.address ? '...' : '🔍 Analyze' }}
              </button>
      ...
      <div class="lg:col-span-1">
        <div class="flex justify-between items-center border-b border-slate-800 pb-2 mb-2">
           <h3 class="text-slate-500 font-bold uppercase tracking-widest text-sm">🗑️ Rejected ({{ rejectedTokens.length }})</h3>
           <button v-if="rejectedTokens.length > 0" @click="rejectedTokens = []" class="text-[10px] text-slate-600 hover:text-red-400 uppercase font-bold">Clear</button>
        </div>

        <div class="bg-black/20 rounded-xl border border-slate-800 h-[300px] lg:h-[500px] overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-slate-700">
